...
def book_appointment():
    """Endpoint to create a hospital appointment using configured Hospital Service.
    Expects JSON with keys: name, email, phone, preferred_date, detection_text, test_data, ai_analysis
    Returns JSON {success, booking_id, message}
    """
    try:
        ...
        # Try to parse JSON-encoded test_data / ai_analysis if they are strings
        import json
        try:
        ...
        result = hospital.book_appointment(booking_payload)
        if result.get('success'):
            return jsonify({'success': True, 'booking_id': result.get('booking_id'), 'message': result.get('message')})
        else:
